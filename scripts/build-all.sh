...
SERVICES=("auth" "attendance" "route" "yard")
...
build_service() {
    ...
    if [ -f "./dev.sh" ]; then
        ...
        echo -e "${YELLOW}[$service]${NC} ⚠ dev.sh not found, skipping clean"
    fi

    # Skip go mod tidy in workspace mode - it causes issues with internal packages
    # The workspace handles dependency resolution automatically
    echo -e "${YELLOW}[$service]${NC} Skipping go mod tidy (using workspace mode)..."

    # Run go generate if generate.go exists
    if [ -f "./generate.go" ]; then
        ...
        echo -e "${YELLOW}[$service]${NC} ⚠ generate.go not found, skipping generation"
    fi
