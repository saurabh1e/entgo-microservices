...
# Function to find brew in common locations
find_brew() {
...
if command -v protoc >/dev/null 2>&1; then
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    ...
    if [[ "$(uname)" == "Darwin" ]]; then
      BREW_CMD=$(find_brew)
      if [[ -n "$BREW_CMD" ]]; then
        run_cmd "$BREW_CMD update && $BREW_CMD install protobuf"
      else
        echo "Homebrew not found. Please install Homebrew first: https://brew.sh/" >&2
        echo "Common locations checked: /opt/homebrew/bin/brew, /usr/local/bin/brew" >&2
        exit 1
      fi
    else
      # Attempt apt-get
...
echo ""
echo "Will install Go plugins: protoc-gen-go and protoc-gen-go-grpc"
...
if $DRY_RUN; then
  echo ""
  echo "Dry run mode: nothing was changed. Re-run with --run to perform the installations."
else
  echo ""
  echo "Installation steps completed (if no errors were reported)."
