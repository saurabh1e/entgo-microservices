...
build_app() {
    ...
    if go build -o bin/test-service .; then
...
clean_schema_methods() {
    ...
    for schema_file in "$SCHEMA_DIR"/*.go; do
        ...
        # e.g., hook "github.com/user/project/test-service/ent/schema_hooks"
        sed_i \
...
check_prerequisites() {
    ...
    if [ ! -f "go.mod" ] || [ ! -f "main.go" ]; then
        log_error "This doesn't appear to be the root of thetest-service project"
        exit 1
    fi
