...
import (
	...
	"github.com/saurabh/entgo-microservices/test-service/graph"
...
func InitializeServer(cfg *config.Config, db *database.DB, jwtService *jwt.Service, redis *database.RedisClient, grpcPool *pkggrpc.ClientPool, userClient *pkggrpc.UserClient) *ServerConfig {
	...
	// Initialize GraphQL resolver with JWT service, Redis client, and gRPC clients
	resolver := graph.NewResolver(db.Client, jwtService, redis.Client, grpcPool, userClient)
